...
public class BlackboardDecorator : Decorator
{
    ...
    protected ETypeObserverAbort _typeObserverAbort;
    ...
    public override EResultDecorator CheckDecorator()
    {
        // YA RETORNA ABORT O CONTINUE, FIJARSE DE IMPLEMENTARLO EN LOS COMPOSITE (Fijarse en los apuntes anotados en el cuaderno negro que me dio NXA)
        ...
        switch (_keyQuery) 
        {
            case EKeyQuery.IsSet:
                if (_typeNotifyObserver == ETypeNotifyObserver.OnChangeResult && _blackboard.GetValue(_blackboardkey) != _lastValue 
                    && _blackboard.GetBlackboardKeysData()[_blackboard.GetIndexBlackboardKeyData(_blackboardkey)].GetIsSet())
                {
                    return EResultDecorator.ABORT;
                }
                else if (_typeNotifyObserver == ETypeNotifyObserver.OnValueChange
                    && _blackboard.GetBlackboardKeysData()[_blackboard.GetIndexBlackboardKeyData(_blackboardkey)].GetIsSet())
                {
                    return EResultDecorator.ABORT;
                }
